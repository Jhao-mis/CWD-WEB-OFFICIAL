...
{
    $stmt = $conn->prepare(
        "SELECT bid_code, bidding_title, upload_date, created_at, file_path, file_name, file_type
 ...
 * Undated documents (e.g. Post Award Information, which never has an
 * upload_date) are skipped here — use bacGroupByQuarterAndMonthFallback()
 * for those instead.
 */
...
/**
 * Same as bacGroupByQuarterAndMonth(), but for categories that don't have
 * an upload_date (e.g. Post Award Information, which is locked/blank by
 * design). Falls back to created_at (when the record was uploaded to the
 * system) so these documents still get bucketed into Q1-Q4 / months.
 */
function bacGroupByQuarterAndMonthFallback(array $docs): array
{
    $quarters = ['Q1' => [], 'Q2' => [], 'Q3' => [], 'Q4' => []];
    $monthNames = [
        1 => 'January', 2 => 'February', 3 => 'March', 4 => 'April',
    ...
    foreach ($docs as $doc) {
        $effectiveDate = $doc['upload_date'] ?: $doc['created_at'] ?? null;
